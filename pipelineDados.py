...
ips = pd.read_excel('ips.xlsx', sheet_name=['Indicadores - 2024', 'Dimensões e Componentes 2024'])
ips = pd.concat(ips.values(), ignore_index=True)
ips = ips.dropna()
ips = ips.drop_duplicates()
ips.columns = ips.iloc[0]
ips = ips[1:].reset_index(drop=True)
#alguns desses dados estão vazios


censo = pd.read_csv('Censo_2022.csv')
censo = censo.dropna()

#juntando o censo e o ips
demanda = pd.merge(ips, censo, on='regiao_adm', how='right')
